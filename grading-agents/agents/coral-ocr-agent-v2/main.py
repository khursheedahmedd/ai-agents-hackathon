...
class SimpleCoralOCRAgent:
    def __init__(self):
        """Initialize Simple Coral OCR Agent with current FastAPI configuration"""
    ...
    async def create_agent(self) -> AgentExecutor:
        """Create simple Coral-compatible OCR agent"""
...
async def main():
    """Main function to run the Simple Coral OCR Agent"""
    logger.info("Starting Simple Coral OCR Agent...")
    ...
    # Create OCR agent
    ocr_agent = SimpleCoralOCRAgent()
    agent_executor = await ocr_agent.create_agent()
    
    logger.info("Simple OCR Agent initialized successfully")
    
    # Simple agent loop for testing
    while True:
        try:
            # For testing, you can input file paths manually
            user_input = input("Enter file path to process (or 'quit' to exit): ").strip()
            ...
            if user_input and os.path.exists(user_input):
                result = await agent_executor.ainvoke({
                    "input": f"Process this document: {user_input}",
                    "agent_scratchpad": []
                })
